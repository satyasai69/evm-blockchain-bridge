...
const mintTokens = async (provider, contract, amount, address) => {
  try {
    ...
    console.log(
      amountd,
      "tokens lock 🔒 on nusa and mint 💸💸💸💸💸 on bsc wallet ",
    );
    ...
    const trxData = {
      ...
      to: "0x8a826644C0c470dA777F292eD8889f7b14FB54E9",//DESTINATION_TOKEN_CONTRACT_ADDRESS,
...
const transferToEthWallet = async (provider, contract, amount, address) => {
  try {
    ...
    console.log(
      amountd,
      "nusa tokens burn 🔥 on BSC and unlock 🔓 on nusa chain 💸💸💸💸💸",
    );
    console.log("By = ", address);
    // console.log('address :>> ', address)
    // console.log('amount :>> ', amount)
    const trx = contract.methods.withdraw(address, amount);
    ...
    console.log(
      amountd,
      `Unlock 🔓🔓🔓 on NURA--- ${process.env.ORIGIN_EXPLORER}${receipt.transactionHash}`,
    );
    console.log(
      "====🛰️====💸💸💸💸💸====🛰️======💸💸💸💸💸======🛰️======💸💸💸💸💸=====🛰️======",
    );
